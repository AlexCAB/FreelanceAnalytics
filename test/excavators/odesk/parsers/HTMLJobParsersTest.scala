...
class HTMLJobParsersTest extends WordSpecLike with Matchers {
  ...
  def foundJobRow(url:String) = FoundJobsRow(
    id = 0,
    oUrl = url,
  ...
  "HTMLJobParsers must:" must{
    ...
    "parse hourly job" in {
      val url = "https://www.odesk.com/jobs/WordPress-Plugin-Developer-Possible-Long-Term-Contract_~0116a80a41a3bd221a"
      val html = getHtml("html\\JobHourly.html")
      //
      val tr = htmlParser.parseJob(html).get
      //
      assert(htmlParser.estimateParsingQuality(foundJobRow(url), Some(tr)) == 1.0)
      //
    ...
    "parse budget job" in {
      val url = "https://www.odesk.com/jobs/Display-ToolTip-SharePoint-2013-DVW-controls_~01b8509ad36fb242d1"
    ...
    "parse applicants rich info " in {
      val url = "https://www.odesk.com/jobs/FACEBOOK-AUTO-POSTER-SCRIPT_~011464d760e8e4eeff"
      ...
      //clientWorks:List[ClientWork]
      assert(tr.clientWorks.size == 0)}
    "parse closed job and hired info and client company info" in {
      val url = "https://www.odesk.com/jobs/~019ba335f65a0f5ace"
      val html = getHtml("html\\JobClosed.html")
      //
      val tr = htmlParser.parseJob(html).get
      //
      assert(htmlParser.estimateParsingQuality(foundJobRow(url), Some(tr)) == 0.7)
      //
      val ct = System.currentTimeMillis()
      //job:Job
    ...
    "parse new format budjet jobs " in {
      val url = "https://www.odesk.com/jobs/Geolocation-code-add-existing-app_%7E0184ecd9f1754aeaac"
      val html = getHtml("html\\JobNewFormat.html")
      //
      val tr = htmlParser.parseJob(html).get
      //
      assert(htmlParser.estimateParsingQuality(foundJobRow(url), Some(tr)) == 0.95)
      //
      val ct = System.currentTimeMillis()
      //job:Job
      val pd = tr.job.postDate.get.getTime + (1000 * 60 * 60 * 24) //Posted  1 day ago
      ...
      //clientWorks:List[ClientWork]
      assert(tr.clientWorks.size == 0)}
  "applicants avg rate error" in { // (should be None)
    val url = "https://www.odesk.com/jobs/Logo-design_%7E01b4638940f9699331"
    val html = getHtml("html\\AvgRateErr.html")
    //
    val tr = htmlParser.parseJob(html).get
    //changes:JobChanges
    assert(tr.jobChanges.applicantsAvg == None)}
  "interviewing avg rate error" in { // (should be 33.33)
    val url = "https://www.odesk.com/jobs/RESTful-API-with-Chunked-HTTP-Streaming-json-Play2-Spray_~014e005d00964bd30a"
