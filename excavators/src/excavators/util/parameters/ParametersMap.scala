  ...
  def getOrElse[T:TypeTag](k:String, e: =>T):T = {
    if(params.contains(k)){
      ...
      typeOf[T] match {
        ...
        case t if t <:< int => try{parseLong(v).toInt.asInstanceOf[T]}catch{case _:Exception => e}
        ...
        case t if t <:< list_int => try{parseList(v).map(parseLong(_)).asInstanceOf[T]}catch{case _:Exception => e}
