package excavators.odesk.sundries

import java.awt.image.BufferedImage
import java.io.File
import java.util.Date
import javax.imageio.ImageIO
import excavators.odesk.db.{DBProvider, Saver}
import excavators.odesk.parsers.HTMLJobParsers
import excavators.odesk.structures.{ParsingErrorRow, FoundJobsRow, FoundBy, ParsedJob}
...
class AbstractWorker(browser:Browser, logger:Logger, db:DBProvider) extends ManagedWorker with TimedTaskExecutor {
  //Parameters
  ...
  protected def estimateQuality(pj:Option[ParsedJob], j:FoundJobsRow, html:Option[String], cd:Date):Double = {
    ...
    if(pq <= htmlParser.notSaveParsingQualityLevel){
      logger.error("[Worker.estimateQuality] Job parsing error, job not save, pq = " + pq + ", url: " + j.oUrl)}
    else if(pq <= htmlParser.errorParsingQualityLevel){
      logger.error("[Worker.estimateQuality] Job parsing error, pq = " + pq + ", url: " + j.oUrl)}
    else if(pq <= htmlParser.wornParsingQualityLevel){
      logger.worn("[Worker.estimateQuality] Job parsing worn, pq = " + pq + ", url: " + j.oUrl)}
